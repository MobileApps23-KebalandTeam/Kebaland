...
public class MyTogle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        toggleManager.toggleListZaakceptowanych = ServiceLocator.Get<IngredientsService>().GetActiveIngredients();
        int totalItems = 0;
        int level = LevelChoice.GetStartedLevel();
        switch (level)
        {
            case 0:
                totalItems = 4;
    ...
    public void OnToggleValueChangedMeat() //metoda użyana przy kliku na toglle
    {
        ...
        if (LevelChoice.GetStartedLevel() == 0) //sprawdzamy czy to jest pierwszy wybur gracza
        {
            if (temat != toggleManager
                    .toggleTemat) //sprawdzamy czy temat wybranego pojedynczego toggle nie jest taka sama jak i aktywna tema(tema poprzednio wybranego toggle)
            {
                toggleManager.toggleTemat = temat; // zmienaiamy aktwną teme
                toggleManager.toggleTypeActiv = type;
                bool numberExists =
                    toggleManager.myList.Exists(item => item.Item1 == temat); //sprawdza czy temat już byl używany 
                if (numberExists) //jeśli tak
                {
                    ...
                    Tuple<int, Toggle, Image, IngredientType>
                        firstPair = toggleManager.myList[index]; //pobiera pare podanego indeksu tematu 

                    firstPair = new Tuple<int, Toggle, Image, IngredientType>(temat, toggleManager.togleActivMeat,
                        toggleManager.krzyzykActiv,
                        type); // modyfikacja danych pary nowym wybranym toggle tego tematu i jego krzyżykiem  
                ...
                else //jeśli temat był wybrany pierwszy raz
                {
                    ...
                    toggleManager.AddTemat(temat, toggleManager.togleActivMeat, toggleManager.krzyzykActiv,
                        type); //dodanie tematu aktywnego toggle i krzyżyka do listy tematów 
                }
            }
            else // jeśli teamat nie zmienial się (wybrany toggle jest z tego samego tematu)
            {
                ...
                {
                    if (toggleManager.toggleCounter <=
                        0) //sprawdzamy jeśli toggle jest wybrany i wyłączony więcej niż 0 MOZE NIE TRZEBA SPRAWDZAĆ!
                    {
                        if (toggle != null && toggleManager != null)
                        {
                            toggleManager.RemoveToggle(toggle, type);
                ...
                firstPair = new Tuple<int, Toggle, Image, IngredientType>(temat, toggleManager.togleActivMeat,
                    toggleManager.krzyzykActiv, type);
        ...
        {
            toggleGroup.enabled =
                false; //robimy togleGroup nie actywnym żeby moglibyśmy wybierać więcej niż jeden toggle z kategoriji
            // Debug.Log("TOOOGGG GROUP " + (toggleGroup.enabled));
            if (licznik.licznikPoprzedni >
                0) //sprawdzamy licznik jeśli on jest więkzsy za 0 możmy wybierać różne toggle
            {
                if (toggle.isOn)
                {
                    toggleManager.AddToggle(toggle, type);
            ...
            else
            {
                ...
                {
                    //jeśli nie to toggle zostaje się wylączonym
                    toggle.isOn = false;
                }
            }
        }
